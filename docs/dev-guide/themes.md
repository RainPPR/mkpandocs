...
[docs_dir]. Within `mkpandocs.yml`, set the [`theme.custom_dir`][custom_dir]
...
... you would include the following settings in `mkpandocs.yml` to use the custom theme
directory:
...
> be defined in the [theme] configuration option in the `mkpandocs.yml` file.
>
...
The body content from each page specified in `mkpandocs.yml` is inserted using the
...
`mkpandocs.yml` config file. While you can use any config option, some commonly
...
defined for the plugin in the user's `mkpandocs.yml` config file under
...
in their mkpandocs.yml and the one on the right is the directory containing your
theme files.
...
And the user could override the default in their project's `mkpandocs.yml` config
file:
...
> set it in `mkpandocs.yml` then it will default to `en` (English). The value
