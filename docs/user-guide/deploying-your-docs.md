...
properdocs gh-deploy --config-file ../my-project/mkpandocs.yml --remote-branch master
```

Note that you need to explicitly point to the `mkpandocs.yml` configuration file as
...
(defaults to `"site"`) in your `mkpandocs.yml` config file. Generally, you will
