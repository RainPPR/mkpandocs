...
def get_deps(
    ...
    """
    Print PyPI package dependencies inferred from a mkpandocs.yml file based on a reverse mapping of known projects.

    Args:
        config_file: Non-default mkpandocs.yml file - content as a buffer, or path.
        projects_file: File/buffer that declares all known MkPandocs-related projects.
    ...
    if all(c not in cfg for c in ("site_name", "theme", "plugins", "markdown_extensions")):
        log.warning(f"The file {config_file!r} doesn't seem to be a mkpandocs.yml config file")
    else:
        if _dig(cfg, "theme.locale") not in (_NotFound, "en"):
            packages_to_install.add("mkpandocs[i18n]")
        else:
            packages_to_install.add("mkpandocs")
