...
def get_pandoc_toc(markdown_source):
    """Return TOC generated by parsing Pandoc HTML output."""
    import pypandoc
    from bs4 import BeautifulSoup

    html = pypandoc.convert_text(
        markdown_source, to='html5', format='markdown', extra_args=['--quiet']
    )
    ...
    for tag in soup.find_all(['h1', 'h2', 'h3', 'h4', 'h5', 'h6']):
        ...
        toc_tokens.append({'level': level, 'id': anchor_id, 'name': name, 'children': []})

    return toc_tokens


# Legacy alias for backward compatibility
get_markdown_toc = get_pandoc_toc
...
def load_config(config_file_path: str | None = None, **cfg) -> ProperDocsConfig:
    ...
    conf = ProperDocsConfig(
        config_file_path=config_file_path or os.path.join(path_base, 'mkpandocs.yml')
    )
    conf.load_dict(cfg)
