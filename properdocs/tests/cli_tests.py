...
class CLITests(unittest.TestCase):
    ...
    def test_serve_config_file(self, mock_serve):
        result = self.runner.invoke(
            cli.cli, ["serve", "--config-file", "mkpandocs.yml"], catch_exceptions=False
        )
    ...
    def test_build_config_file(self, mock_build, mock_load_config):
        result = self.runner.invoke(
            cli.cli, ['build', '--config-file', 'mkpandocs.yml'], catch_exceptions=False
        )
    ...
    def test_gh_deploy_config_file(self, mock_gh_deploy, mock_build, mock_load_config):
        result = self.runner.invoke(
            cli.cli, ['gh-deploy', '--config-file', 'mkpandocs.yml'], catch_exceptions=False
        )
